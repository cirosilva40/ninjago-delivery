    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={`max-w-2xl ${isLight ? 'bg-white text-gray-900' : 'bg-slate-900 text-white'} border-${isLight ? 'gray-200' : 'slate-700'}`}>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {produto.opcoes_personalizacao.map((grupo, grupoIndex) => {
                ...
                return (
                  <div 
                    key={grupoIndex} 
                    className={`p-4 rounded-xl border ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-slate-800/30 border-slate-700'}`}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h5 className={`font-semibold ${isLight ? 'text-gray-900' : 'text-white'}`}>
                          {grupo.nome_grupo}
                      ...
                      {grupo.max_selecoes && (
                        <Badge 
                          variant="outline" 
                          className={selecoesGrupo.length >= grupo.max_selecoes ? 'border-orange-500 text-orange-500' : ''}
                        >
          ...
          {/* Preço e Ação */}
          <div className={`flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-6 rounded-xl ${isLight ? 'bg-gray-50 border border-gray-200' : 'bg-slate-800/50 border border-slate-700'}`}>
            <div>
            ...
            <Button
              ...
              size="lg"
              className="bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white text-base sm:text-lg px-6 sm:px-8 w-full sm:w-auto"
            >
